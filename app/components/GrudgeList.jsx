...
export default function GrudgeList(){
    ...
    return (
        <div style={{margin: '20px'}}>
            <h3 data-testid="grudge-list-title" style={{margin: '20px 0px'}}>{title}</h3>
            <div>
            <TextField 
            label="Add Grudge" 
            variant="filled" 
            ...
            <Button data-testid="add-grudge" onClick={addGrudge}>
                Add Grudge
            </Button>
            <ul data-testid="grudge-list" style={{color: 'white', listStyleType: 'none'}}>
                {
                    grudges.length > 0 && (
                        grudges.map((g) => {
                           return ( 
            ...
            {
                grudges.length > 0 && <Button data-testid="clear-grudges" onClick={clearGrudges}>Clear</Button>
            }
        </div>
    )
}